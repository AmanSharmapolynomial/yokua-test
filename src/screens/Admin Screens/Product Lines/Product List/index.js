...
const ProductList = () => {
  ...
  const handleProductClick = (id, name, isStatic, route) => {
    if (isStatic) {
      navigate(`/admin/${route}`, {
        state: { header: name, previousPage: [{ link: location.pathname, name: 'Product Line' }] },
      })
    } else {
      getSubProducts(id).then(data => {
        if (data && Array.isArray(data))
          if (!data[0].sub_product_name === 'proxy') {
            /**
             * If Product Does not have Sub Product - Navigate to Product Detail else Sub Products Page
             */
            // navigate(`/admin/product-detail/${id}`, {
            //   state: {
            //     header: name,
            //     previousPage: [{ link: location.pathname, name: 'Product Line' }],
            //   },
            // })

            navigate(`/admin/sub-products/${id}`, {
              state: {
                header: name,
                previousPage: [{ link: location.pathname, name: 'Product Line' }],
              },
